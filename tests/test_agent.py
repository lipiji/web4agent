...
class TestAgentReadUrl:
    ...
    async def test_returns_dict(self):
        with patch("web4agent.agent.read_url", AsyncMock(return_value=_ok("https://x.com"))):
    ...
    async def test_content_truncated(self):
        ...
        with patch("web4agent.agent.read_url", AsyncMock(return_value=r)):
            from web4agent.agent import agent_read_url
            result = await agent_read_url("https://x.com")
        from web4agent.config import AGENT_MAX_CONTENT_CHARS
        assert result["content"] is not None
        assert len(result["content"]) <= AGENT_MAX_CONTENT_CHARS + 50  # allow for ellipsis

    @pytest.mark.asyncio
    async def test_failure_result(self):
        with patch("web4agent.agent.read_url", AsyncMock(return_value=_fail("https://x.com", "DNS error"))):
            from web4agent.agent import agent_read_url
            result = await agent_read_url("https://x.com")
...
class TestAgentReadUrls:
    ...
    async def test_returns_summary_dict(self):
        ...
        with patch("web4agent.agent.read_many", AsyncMock(return_value=results)):
            from web4agent.agent import agent_read_urls
    ...
    async def test_empty_urls_list(self):
        with patch("web4agent.agent.read_many", AsyncMock(return_value=[])):
            from web4agent.agent import agent_read_urls
